...
A final note on lazy lookups: it doesn't seem like their results are used immediately. The aforementioned subrecords and some in-game classes used to load content (e.g. QueuedFileEntry, QueuedTexture) use a lazy lookup to find the metadata (BSAEntry pointer) for an archived file (size and offset), and then later try to retrieve the file using that metadata (via a call to Archive::GetFileByEntry). The subrecords don't store any information that could be used for a fallback; it's possible that they're used to pre-load textures if possible, with "proper" texture loads done later if the pre-loading fails.
